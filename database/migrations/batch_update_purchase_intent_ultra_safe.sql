...
DO $$
DECLARE
  v_batch_size INTEGER := 1000;  -- 极小批次，确保不超时
  v_updated INTEGER;
  v_total_updated INTEGER := 0;
  v_iteration INTEGER := 0;
  v_max_iterations INTEGER := 210;  -- 203,062 ÷ 1000 ≈ 203 次
BEGIN
  RAISE NOTICE '🚀 开始批量更新 Purchase Intent（超安全模式）...';
  RAISE NOTICE '批次大小: % 条', v_batch_size;
  RAISE NOTICE '延迟时间: 1.5 秒/批';
  RAISE NOTICE '预计时间: 约 5-6 分钟';
  RAISE NOTICE '';
  ...
  LOOP
    ...
    -- 每 10 批显示一次进度，避免输出太多
    IF v_iteration % 10 = 0 OR v_updated = 0 THEN
