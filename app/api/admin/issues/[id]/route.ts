...
export async function DELETE(
  ...
  try {
    ...
    if (fetchError || !issue) {
      return NextResponse.json({ error: '售后问题不存在' }, { status: 404 })
    }
    ...
    if (deleteError) {
      console.error('删除售后问题失败:', deleteError)
      return NextResponse.json(
        { error: '删除售后问题失败', details: deleteError.message },
        { status: 500 }
      )
    }
