 ...
 * 
 * 注意：Next.js 不支持路径中的动态段，所以使用 catch-all 路由 [...n]
 * 实际 URL 格式：/sitemaps/tier1-1.xml → params.n = ['tier1-1.xml']
 */
export async function GET(
  ...
  // 解析分片索引：从 ['tier1-1.xml'] 提取 1
  const pathParts = params.n || []
  const lastPart = pathParts[pathParts.length - 1] || ''
  
  // 匹配 tier1-{数字}.xml
  const match = lastPart.match(/^tier1-(\d+)\.xml$/)
  if (!match) {
    return new NextResponse('Invalid sitemap path. Expected: /sitemaps/tier1-{n}.xml', { status: 400 })
  }
