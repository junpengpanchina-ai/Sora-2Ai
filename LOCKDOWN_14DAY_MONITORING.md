...
- **数据来源**：手填（GSC 5 指标 + URL 抽检）。不接 GSC API，要的是纪律。
- **今日未填报提醒**：若当天无填报记录，Admin 锁仓面板会显示「今日尚未填报，请完成 GSC 5 指标后点击下方『今日填报』。」打开 Admin 即相当于一次每日反馈。
- **可选（锁仓期不建议）**：GSC API 自动取抓取/发现/索引/sitemap；或 Vercel Cron + 邮件/飞书每日提醒。见需求再做。
