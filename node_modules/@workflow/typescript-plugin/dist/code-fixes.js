...
exports.getCodeFixes = getCodeFixes;
const utils_1 = require("./utils");
function getCodeFixes(fileName, start, end, errorCode, program, ts) {
    const fixes = [];
...
function getDirectiveTypoFix(fileName, start, end, program, ts) {
    const fixes = [];
    ...
    const expectedDirective = (0, utils_1.getDirectiveTypo)(typoText);
...
function getDirectWorkflowInvocationFix(fileName, start, end, program, ts) {
    const fixes = [];
    const sourceFile = program.getSourceFile(fileName);
    if (!sourceFile) {
        return fixes;
