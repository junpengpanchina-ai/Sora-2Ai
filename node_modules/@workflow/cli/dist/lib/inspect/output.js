...
const formatTableValue = (prop, value, opts = {}, displaySettings, item) => {
...
const showTable = (data, props, opts = {}) => {
    ...
    for (const item of data) {
        for (const prop of visibleProps) {
            ...
            table.cell(header, formatTableValue(prop, value, opts, displaySettings, item));
        }
        table.newRow();
    }
    // Restore original truncate length
...
const showInspectInfoBox = (resource) => {
    logger.info(`To view details for a ${resource}, use \`workflow inspect ${resource}\` <id>`);
    logger.info(`To view the content of any stream, use \`workflow inspect stream <stream-id>\``);
};
const EXPIRED_DATA_MESSAGE = chalk.gray('<data expired>');
/**
 * Checks if a run has expired data storage
 */
const hasExpiredData = (run) => {
...
const inlineFormatIO = (io, topLevel = true) => {
    ...
    if (io === undefined) {
        ...
        else if (type === 'string' && io.includes('strm_')) {
            value = io;
        }
        else {
    ...
    try {
        ...
        if (opts.json) {
            ...
            if (hasExpiredData(run)) {
                logger.warn("This run's data (input/output/error) has expired and is no longer available.");
            }
...
export const showStream = async (world, streamId, opts = {}) => {
    if (opts.runId || opts.stepId) {
        logger.warn('Filtering by run-id or step-id is not supported when showing a stream, ignoring filter.');
    }
 ...
 */
export const listStreamsByRunId = async (world, opts = {}) => {
    if (opts.withData) {
    ...
    if (opts.stepId) {
        logger.warn('Filtering by step-id is not supported for streams, ignoring filter.');
    }
    let runId = opts.runId;
    if (!runId) {
        logger.warn('No run-id provided. Listing streams for latest run instead.', 'Use --run=<run-id> to filter streams by run.');
