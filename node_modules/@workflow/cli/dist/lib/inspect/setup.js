...
import { inferLocalWorldEnvVars, inferVercelEnvVars, writeEnvVars, } from './env.js';
// Get XDG-compliant cache directory for workflow
const getXDGAppPaths = (app) => {
    return XDGAppPaths(app);
};
const getWorkflowCacheDir = () => {
 ...
 */
export const setupCliWorld = async (flags, version, ignoreLocalWorldConfigError = false) => {
    ...
    if (updateCheck.needsUpdate && updateCheck.latestVersion) {
        boxLines.push('', chalk.cyan(`Update available: ${updateCheck.currentVersion} → ${updateCheck.latestVersion}`), 
        ...
        chalk.gray(`Run: \`[npm|bun|pnpm] i workflow@${updateCheck.latestVersion}\``), chalk.gray(terminalLink('View changelog', 'https://github.com/vercel/workflow/releases')));
    }
    ...
    if (flags.backend === 'vercel' ||
        ...
        try {
            ...
            if (ignoreLocalWorldConfigError) {
                ...
                logger.warn('Failed to find valid local world configuration:', configError);
                return null;
            }
            throw error;
        }
