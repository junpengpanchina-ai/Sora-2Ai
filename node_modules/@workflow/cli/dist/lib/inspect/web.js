...
export const getHostUrl = (webPort) => `http://localhost:${webPort}`;
...
function getWebPackagePath() {
    try {
        ...
        const packageJsonPath = requireFromHere.resolve(`${WEB_PACKAGE_NAME}/package.json`);
        logger.debug(`Resolved web package: ${packageJsonPath}`);
        // Return the directory containing package.json
        return dirname(packageJsonPath);
    }
    catch (error) {
        throw new Error(`Failed to resolve ${WEB_PACKAGE_NAME} package. ` +
 ...
 */
async function isServerRunning(hostUrl) {
    try {
 ...
 */
async function startWebServer(webPort) {
    if (isServerStarting) {
    ...
    try {
        ...
        logger.error(`Server failed to start within ${maxRetries * retryIntervalMs}ms`);
        cleanupServer();
        isServerStarting = false;
        return false;
    }
 ...
 */
function buildWebUIUrl(hostUrl, resource, id, flags) {
    ...
    for (const flagName of ['runId', 'stepId', 'hookId']) {
 ...
 */
export async function launchWebUI(resource, id, flags, _cliVersion) {
    ...
    const isVercelBackend = vercelBackendNames.includes(envVars.WORKFLOW_TARGET_WORLD);
    ...
    if (!useLocalUi && isVercelBackend) {
        logger.info('If you do not want to use the Vercel dashboard, use the --localUi flag or set WORKFLOW_LOCAL_UI=1 in your environment variables.');
    }
    if (isVercelBackend && teamSlug && projectName && !useLocalUi) {
        logger.debug(`Checking Vercel dashboard availability for team: ${teamSlug}, project: ${projectName}`);
        const dashboardUrl = getVercelDashboardUrl(teamSlug, projectName, resource, id);
        ...
        logger.info(chalk.green(`Opening Vercel dashboard for workflows at: ${dashboardUrl}`));
        try {
    ...
    if (alreadyRunning) {
        logger.info(chalk.cyan(`Web UI server is already running on port ${webPort}.`));
    }
    else {
        // Start the server
