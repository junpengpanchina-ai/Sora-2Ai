...
export default class Inspect extends BaseCommand {
    ...
    async run() {
        try {
            ...
            if (resource === 'event') {
                if (id) {
                    this.logError('Event-ID is not supported for events. Filter by run-id or step-id instead. Usage: `workflow inspect events --runId=<id>`');
                    process.exit(1);
                }
...
function toInspectOptions(flags) {
    return {
        json: flags.json,
        runId: flags.runId,
        stepId: flags.stepId,
