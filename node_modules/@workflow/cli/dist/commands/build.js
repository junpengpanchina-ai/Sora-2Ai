...
export default class Build extends BaseCommand {
    ...
    async run() {
        ...
        let buildTarget = flags.target;
        if (args.target && !flags.target) {
            buildTarget = args.target;
            this.logWarn('Using positional argument for target is deprecated. Use --target flag instead.');
        }
        if (flags['workflow-manifest'] &&
            !flags['workflow-manifest'].match(/\.(json|js|cjs|mjs)$/)) {
            throw new Error(`Invalid --workflow-manifest provided, must end in .json or .js. Received: ${flags['workflow-manifest']}`);
        }
        // Validate build target
        if (!isValidBuildTarget(buildTarget)) {
            this.logWarn(`Invalid target "${buildTarget}". Using default "standalone".`);
