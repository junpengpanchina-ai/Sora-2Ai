...
(async function() {
  ...
  window.fullDiagnostics = async function() {
    ...
    try {
      ...
      if (authData.error) {
        ...
        if (authData.debug) {
          console.log('   🔍 调试信息:', JSON.stringify(authData.debug, null, 2))
        }
      } else {
  ...
  // 3. 测试创建会话
  window.testCreateSession = async function(title = '测试会话') {
  ...
  // 4. 测试发送消息
  window.testSendMessage = async function(sessionId = null, message = '测试消息') {
    console.log('💬 测试发送消息...')
    try {
...
💡 示例：
   await testCreateSession('我的第一个会话')
